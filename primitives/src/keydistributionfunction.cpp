#include "../headers/keydistributionfunction.hpp"


void HKDF::ithRound(int outLen, const vector<byte> & iv, int hmacLength, vector<byte> & outBytes, vector<byte> & intermediateOutBytes) {
	...
	for (int i = 2; i <= rounds; i++) {
		...
		this->hmac->computePRFBlock(currentInBytes, 0, currentInBytesSize, intermediateOutBytes, 0);
...
void HKDF::initialRound(vector<byte>& outBytes, const vector<byte> & iv, vector<byte> & intermediateOutBytes, int outLength) {
	// Round 1.
	vector<byte> initialRoundInput; //Data for the creating K(1).
	int initialRoundSize;
	if (iv.size() > 0) {
		initialRoundSize = iv.size() + 1;
		initialRoundInput.resize(initialRoundSize);
		// Copy the CTXInfo - iv.
		memcpy(initialRoundInput.data(), iv.data(), iv.size());
	}
	else {
		initialRoundSize = 1;
		initialRoundInput.resize(initialRoundSize);
	...
	initialRoundInput[initialRoundSize - 1] = (byte)1;
	...
	hmac->computePRFBlock(initialRoundInput, 0, initialRoundSize, intermediateOutBytes, 0);
...
SecretKey HKDF::deriveKey(const vector<byte> & entropySource, int inOff, int inLen, int outLen, const vector<byte>& iv) {
	...
	hmac->setPRFKey(key);
	int hmacLength = hmac->getPRFBlockSize(); // The size of the output of the hmac.
